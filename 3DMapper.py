...
#input variables
bins=4
grasspresent=[0,1,0]  #Presence of grass on brown,ochre,unsorted variable which you can name as('add grass')
gaussblur=15
inpaint=0 #May not be implemented

# load the image and convert it from BGR to RGB so that
# we can dispaly it with matplotlib
image = cv2.imread("C:/Users/Axiomatize/Desktop/Images to test/4bf4d9b1bc4ca9fa02cff2ceae4b71ea.jpg")
...
while(q<4):
    ...
    if Cmap[0][q][0]>ranges[k][0][0] and Cmap[0][q][0]<ranges[k+1][1][0]:
        ...
        if f==2:
            ...
            if(yorb[0][2]<yorb[1][2]):   #Lower Value of V is brown
                ...
                blur = cv2.GaussianBlur(closing, (0,0), sigmaX=3, sigmaY=3, borderType = cv2.BORDER_DEFAULT)
                closing = skimage.exposure.rescale_intensity(blur, in_range=(127.5,255), out_range=(0,255))
                ...
                closing = pystripe.filter_streaks(closing, sigma=[128, 256], level=7, wavelet='db2')
                ...
                final = cv2.GaussianBlur(closing, (31, 31), cv2.BORDER_DEFAULT)
                final= cv2.filter2D(final,-1,mean_filter_kernel)   
                final = cv2.medianBlur(final,11)
                final = pystripe.filter_streaks(final, sigma=[128, 256], level=7, wavelet='db2')

                noise_img = random_noise(final, mode='gaussian', var=0.05**2)
                ...
                closing = cv2.GaussianBlur(closing, (31, 31), cv2.BORDER_DEFAULT)
                noise_img = random_noise(closing, mode='gaussian', var=0.05**2)
                #closing = (255*noise_img).astype(np.uint8)
    ...
    if q==3:
        ...
        if colournotsorted!=0:
            print("colournotsorted",Cmap[0][colournotsorted-1],colournotsorted)
            ...
            clcopy=closing.copy()
            #_,closing = cv2.threshold(closing,0.,255.,cv2.THRESH_OTSU)
            noise_img = random_noise(closing, mode='poisson')#, var=0.05**2
            closing = (255*noise_img).astype(np.uint8)
            closing=np.array(closing,dtype='uint8')
