...
def script(image):
    ...
    # input variables
    bins = 4
    # Presence of grass on brown,ochre,unsorted variable which you can name as('add grass')
    grasspresent = [0, 1, 0]
    gaussblur = 15
    inpaint = 0  # May not be implemented
    ...
    FY = np.zeros(image.shape[:2], dtype='uint8')
    FB = np.zeros(image.shape[:2], dtype='uint8')
    FG = np.zeros(image.shape[:2], dtype='uint8')
    FUnsort = np.zeros(image.shape[:2], dtype='uint8')
    ...
    cv2.imwrite('Uniform.jpg', out)

    out = cv2.GaussianBlur(out, (15, 15), cv2.BORDER_DEFAULT)
    ...
    mean_filter_kernel = np.ones((sliding_window_size_x, sliding_window_size_y),
                                 np.float32)/(sliding_window_size_x*sliding_window_size_y)
    bcount, ycount, blcount, gcount = 0, 0, 0, 0
    ...
    while(q < 4):
        ...
        if Cmap[0][q][0] > ranges[k][0][0] and Cmap[0][q][0] < ranges[k+1][1][0]:
            ...
            if f == 2:
                ...
                if(yorb[0][2] < yorb[1][2]):  # Lower Value of V is brown
                    ...
                    print("L:", L)
                    Sigma = np.array([4, 4, 4][::-1], dtype="uint8")
                    print(L)
                    ...
                    closing = cv2.morphologyEx(
                        z, cv2.MORPH_CLOSE, kernel, iterations=1)
                    # closing=cv2.bitwise_not(closing)
                    #closing = cv2.GaussianBlur(closing, (15, 15), cv2.BORDER_DEFAULT)
                    blur = cv2.GaussianBlur(
                        closing, (0, 0), sigmaX=3, sigmaY=3, borderType=cv2.BORDER_DEFAULT)
                    closing = skimage.exposure.rescale_intensity(
                    ...
                    for c in cnts:  # For First contour(Mountain)
                        ...
                        if M['m00'] != 0:
                            ...
                            for j in range(1, 101):  # For a contour at a particular r
                                ...
                                # Draw contours for every r
                                cv2.drawContours(
                                    closing, [contourClone], -1, color, 2)
                    # Can the above line be taken out of the loop by summing each tuple?

                    closing = np.array(closing, dtype='uint8')
                    closing = cv2.GaussianBlur(
                        closing, (15, 15), cv2.BORDER_DEFAULT)
                    closing = cv2.filter2D(closing, -1, mean_filter_kernel)
                    closing = pystripe.filter_streaks(
                        closing, sigma=[128, 256], level=7, wavelet='db2')
                    ...
                    final = cv2.GaussianBlur(
                        closing, (31, 31), cv2.BORDER_DEFAULT)
                    final = cv2.filter2D(final, -1, mean_filter_kernel)
                    final = cv2.medianBlur(final, 11)
                    final = pystripe.filter_streaks(
                        final, sigma=[128, 256], level=7, wavelet='db2')

                    noise_img = random_noise(
                        final, mode='gaussian', var=0.05**2)
                    #final = (255*noise_img).astype(np.uint8)
                        ...
                        if M['m00'] != 0:
                            ...
                            for j in range(1, 101):  # For a contour at a particular r
                                ...
                                cv2.drawContours(
                                    closing, [contourClone], -1, color, 2)
                    """for i in range(3):
                    final = cv2.blur(closing, (31, 31), cv2.BORDER_DEFAULT)"""
                    closing = cv2.GaussianBlur(
                        closing, (31, 31), cv2.BORDER_DEFAULT)
                    noise_img = random_noise(
                        closing, mode='gaussian', var=0.05**2)
                    #closing = (255*noise_img).astype(np.uint8)
                    closing = cv2.medianBlur(closing, 11)
                    closing = cv2.filter2D(closing, -1, mean_filter_kernel)
                    closing = pystripe.filter_streaks(
                        closing, sigma=[128, 256], level=7, wavelet='db2')
        ...
        if high and low:
            ...
            print("Works")
        if q == 3:
            colournotsorted = (1+2+3+4)-sum(colours_used)
            if colournotsorted != 0:
                print("colournotsorted",
                      Cmap[0][colournotsorted-1], colournotsorted)
                k = 3  # Treat unsorted colour as Green

                L = np.array(Colours[colournotsorted-1][::-1], dtype="uint8")
                print("L:", L)
                Sigma = np.array([4, 4, 4][::-1], dtype="uint8")
                print(L)
                z = np.zeros(image.shape[:2])
                ...
                #_,closing = cv2.threshold(closing,0.,255.,cv2.THRESH_OTSU)
                noise_img = random_noise(
                    closing, mode='poisson')  # , var=0.05**2
                closing = (255*noise_img).astype(np.uint8)
                closing = np.array(closing, dtype='uint8')
                cv2.imwrite("Unsorted.jpg", closing)
                gre = cv2.imread("Unsorted.jpg", 0)
                FUnsort = cv2.bitwise_and(gre, clcopy)
                cv2.imwrite("UnsortedFinal.jpg", FUnsort)
                # print("Works")

        q = q+1
    final2 = cv2.bitwise_or(FUnsort, FG)
    final2 = cv2.bitwise_or(final2, FY)
...
def process_picture(request):
    with request.FILES["image"] as file:
